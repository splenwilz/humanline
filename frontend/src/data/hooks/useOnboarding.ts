...
import {
  onboardingApi,
  domainValidation as domainValidationUtils,
} from '../api/onboarding'
...
export const useOnboarding = () => {
  const {
    ...
    isLoading,
    mutate: refetch,
  } = useSWR(createCacheKey.onboarding(), () => onboardingApi.get(), {
    ...
    shouldRetryOnError: (error) => {
      ...
      if (
        ...
        error.message.includes('No onboarding record found')
      ) {
        return false
      }
...
export const useDomainAvailability = (
  ...
  const [availability, setAvailability] =
    useState<DomainAvailabilityResponse | null>(null)
...
export const useOnboardingValidation = (formData: OnboardingFormData) => {
  ...
  const validateField = useCallback(
    (field: keyof OnboardingFormData, value: string): boolean => {
      let error = ''
      ...
      switch (field) {
        ...
        case 'companyDomain': {
          const domainValidationResult =
            domainValidationUtils.validateFormat(value)
